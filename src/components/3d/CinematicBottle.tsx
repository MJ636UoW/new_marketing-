...
interface CinematicBottleProps {
  ...
  scrollVelocity: number; // Speed of scroll
  isMobile?: boolean;
}
...
export function CinematicBottle({
  ...
  // Particle counts
  const innerParticleCount = isMobile ? 50 : 180;
  const atmosParticleCount = isMobile ? 80 : 300;
  ...
  // Generate atmospheric cloud particles (Act 3)
  const [atmosPos, atmosSpd] = useMemo(() => {
    const pos = new Float32Array(atmosParticleCount * 3);
    const spd = new Float32Array(atmosParticleCount);
  ...
  useFrame((state, delta) => {
    ...
    // Map scrollProgress (0.0 to 1.0) into the 6 Acts smoothly with math smoothstep:
    // Act 1: 0.00 - 0.16 (Containment)
    // Act 2: 0.16 - 0.33 (Separation / Exploded view)
    // Act 3: 0.33 - 0.50 (Atmosphere expansion)
    // Act 4: 0.50 - 0.66 (Orbit camera 360)
    // Act 5: 0.66 - 0.83 (Immersion camera zoom inside)
    // Act 6: 0.83 - 1.00 (Return to hero state)

    const p = Math.max(0, Math.min(1, scrollProgress));

    // Calculate Act smooth weights
    ...
    // --- ACT 2: SEPARATION (EXPLODED VIEW OFFSETS) ---
    ...
    // --- ACT 3: ATMOSPHERE (PARTICLE EXPANSION) ---
    ...
    // --- ACT 4: ORBIT & CAMERA TRAJECTORY ---
    let targetCamX = 0;
    let targetCamY = 0;
    let targetCamZ = 5.8;

    if (p >= 0.45 && p < 0.68) {
      // Act 4: 360 Camera Orbit Arc around bottle
      ...
      // Act 5: Immersion (Push camera inside bottle liquid core)
      ...
      // Act 1 & 6: Standard Hero Camera distance
      targetCamZ = 5.8;
      targetCamX = 0;
      targetCamY = 0;
    }

    // Dynamic Camera Motion Blur (FOV Expansion during fast velocity)
    const velocityNormalized = Math.min(scrollVelocity * 0.04, 1.0);
    const targetFov = 42 + velocityNormalized * 12; // FOV stretches on fast scroll
    ...
    // Continuous idle rotation when idle (unless scrolling fast)
    if (groupRef.current && scrollVelocity < 2) {
      groupRef.current.rotation.y += 0.003;
    }

    // Dynamic Section-Specific Lighting Changes
    ...
    if (intLightRef.current) {
      intLightRef.current.intensity = THREE.MathUtils.lerp(
        ...
        1.5 + act5Factor * 4.0, // High internal glow during Immersion
        0.1
      );
    }

    // Internal particles breathing drift
  ...
  return (
    <group ref={groupRef} position={[0, 0, 0]}>
      ...
      {/* --- 7. ATMOSPHERIC CLOUD PARTICLES (ACT 3 EXPANSION) --- */}
